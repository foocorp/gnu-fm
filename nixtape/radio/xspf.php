...
if(ereg('l(ast|ibre)fm://globaltags/(.*)', $url, $regs)) {
	...
	$res = $adodb->Execute('SELECT Track.name, Track.artist_name, Track.album_name FROM Track INNER JOIN Loved_Tracks ON Track.artist_name=Loved_Tracks.artist AND Track.track_name=Loved_Tracks.name WHERE Loved_Tracks.userid=' . $user->id . ' Track.streamurl<>\'\' AND Track.streamable=1');
} else {
	die("FAILED\n"); // this should return a blank dummy playlist instead
}
